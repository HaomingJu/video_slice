...
std::vector<Slice_info>& SearchMP4::getMP4Path(const int64_t& start_point, const int& mp4_len)
{
    Slice_info info;
...
{
    std::vector<std::string>::iterator iter = find(m_search_path.begin(), m_search_path.end(), path);
...
{
    for(std::vector<std::string>::iterator iter = m_search_path.begin(); iter != m_search_path.end(); ++iter)
    {
        LOGD_T(MODULE_TAG) << "path: " << *iter;
    }
}
bool SearchMP4::getFiles(std::string& path, std::vector<std::string>& v_mp4_path)
{
    // 正则匹配
